...
--  See `:help vim.hl.hl_op()` (0.13+) / `:help vim.hl.on_yank()` (older)
vim.api.nvim_create_autocmd("TextYankPost", {
  desc = "Highlight when yanking (copying) text",
  group = vim.api.nvim_create_augroup("kickstart-highlight-yank", { clear = true }),
  callback = function()
    -- vim.hl.on_yank is deprecated on 0.13+ in favor of vim.hl.hl_op;
    -- fall back for stable nvim (0.10 only has vim.highlight)
